...
func New(out io.Writer, o *Options) *developHandler {
	...
	if o != nil {
		...
		if o.HandlerOptions != nil {
			...
			if o.Level == nil {
				h.opts.Level = logx.LevelInfo
			} else {
				h.opts.Level = o.Level
			}
		} else {
			h.opts.HandlerOptions = &logx.HandlerOptions{
		...
		h.opts = Options{
			HandlerOptions:    &logx.HandlerOptions{Level: logx.LevelInfo},
...
func (h *developHandler) Enabled(ctx context.Context, l logx.Level) bool {
	return l >= h.opts.Level.Level()
}

func (h *developHandler) WithGroup(s string) logx.Handler {
	if s == "" {
		return h
	}
...
func (h *developHandler) WithAttrs(as []logx.Attr) logx.Handler {
	if len(as) == 0 {
		return h
	}
...
func (h *developHandler) formatSourceInfo(b []byte, r *logx.Record) []byte {
	if h.opts.AddSource {
		...
		if h.opts.ReplaceAttr != nil {
			attr := h.opts.ReplaceAttr([]string{}, logx.Any(logx.SourceKey, s))
			if attr.Key == "" {
				b = append(b, '\n')
				return b
			}
...
func (h *developHandler) levelMessage(b []byte, r *logx.Record) []byte {
	var ls string
	if h.opts.ReplaceAttr != nil {
		a := h.opts.ReplaceAttr(nil, logx.Any(logx.LevelKey, r.Level))
		ls = a.Value.String()
		if a.Key != "level" {
			r.AddAttrs(a)
		}
	} else {
		ls = logx.LevelStringWithDelta(r.Level)
	}
...
func (h *developHandler) processAttributes(b []byte, r *logx.Record) []byte {
	var as attributes
	r.Attrs(func(a logx.Attr) bool {
...
func (h *developHandler) colorize(b []byte, as attributes, l int, group []string, vi visited) []byte {
	...
	for _, a := range as {
		...
		case logx.KindTime, logx.KindDuration:
		...
		if a.Value.Kind() != logx.KindGroup {
			b = append(b, '\n')
		}
	}
